...
def connectivity_test(test_type):
    ...
    def test_code():
        ...
        for line in Lines:
            ...
            elif 'Connection refused' in a1:
                fail_count4 = fail_count4 + 1

                s8 = re.compile(r'connect to [A-Za-z_-]+')
                m8 = s8.search(a1)
                temp_host5 = m8.group().lstrip('connect to ')
                host5 = temp_host5+':'

                s9 = re.compile(r'port [0-9]+')
                m9 = s9.search(a1)
                port5 = m9.group(0).lstrip("port ")

                fail_host4.append(host5+port5)


        fail_count = fail_count1 + fail_count2 + fail_count3 + fail_count4
